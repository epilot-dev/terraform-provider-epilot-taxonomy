...
// RelationAttributeDataClassification - Data classification of the attribute, used by anonymized responses (`?anonymize=true` or tokens minted with `anonymize: true`).
//
...
type RelationAttributeDataClassification string

const (
	RelationAttributeDataClassificationPublic RelationAttributeDataClassification = "public"
	RelationAttributeDataClassificationPii    RelationAttributeDataClassification = "pii"
)

func (e RelationAttributeDataClassification) ToPointer() *RelationAttributeDataClassification {
	return &e
}
func (e *RelationAttributeDataClassification) UnmarshalJSON(data []byte) error {
	var v string
	if err := json.Unmarshal(data, &v); err != nil {
		return err
	}
	...
	case "public":
		fallthrough
	case "pii":
		*e = RelationAttributeDataClassification(v)
		return nil
	default:
		return fmt.Errorf("invalid value for RelationAttributeDataClassification: %v", v)
	}
}
...
type RelationAttribute struct {
	...
	// When unset, built-in defaults apply based on the attribute type (email, phone, address, payment) and a curated list of well-known PII fields.
	//
	DataClassification *RelationAttributeDataClassification `json:"data_classification,omitempty"`
...
func (r *RelationAttribute) GetDataClassification() *RelationAttributeDataClassification {
	if r == nil {
		return nil
	}
